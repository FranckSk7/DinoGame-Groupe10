...
type TypeObstacle = "cactus" | "pterodactyle";
...
export class Obstacle {
  ...
  // --- Images ---
  private imgCactus: HTMLImageElement;
  private imgPtero: HTMLImageElement;
  private imagesChargees: boolean;

  // --- Chargement images (statique, partagé entre toutes les instances) ---
  private static _imgCactus: HTMLImageElement | null = null;
  private static _imgPtero: HTMLImageElement | null = null;
  private static _imagesChargees: boolean = false;

  constructor(canvas: HTMLCanvasElement, vitesse: number) {
    ...
    if (!Obstacle._imgCactus) {
      ...
      let nbChargees = 0;
      const onLoad = () => { nbChargees++; if (nbChargees === 2) Obstacle._imagesChargees = true; };
  ...
  // ----------------------------------------------------------
  setVitesse(nouvelleVitesse: number): void {
...
export class GestionnaireObstacles {
  ...
  private vitesse: number;
  private frameDepuisDernier: number;
  private intervalleMin: number;
  private intervalleMax: number;
  private prochaineApparition: number;
  ...
  private genererIntervalle(): number {
    return Math.floor(
      this.intervalleMin + Math.random() * (this.intervalleMax - this.intervalleMin)
    );
  }
  ...
  // ----------------------------------------------------------
  mettreAJourVitesse(score: number): void {
