...
const canvas        = document.getElementById("canvas-jeu") as HTMLCanvasElement;
...
const nuages: { x: number; y: number; vitesse: number }[] = [
...
type EtatJeu = "accueil" | "enCours" | "gameOver";
let etatJeu: EtatJeu = "accueil";
let frameId: number;
...
function dessinerSol(): void {
  positionSol -= VITESSE_SOL;
  if (positionSol <= -canvas.width) positionSol = 0;
